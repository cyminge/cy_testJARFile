...
public class SignatureUtil {
	...
	public static String[] getPublicKeyString(PackageInfo pi) {
		...
		for (int i = 0; i < pubKeys.length; i++)
			strPubKeys[i] = Base64.encodeToString(pubKeys[i].getEncoded(), Base64.DEFAULT);
		return strPubKeys;
	}
	...
	private static PublicKey[] getInstalledAppPublicKey(Context context, String packageName) {
	...
	public static boolean verifySignature(Context context, String packageName, String filePath) {
		boolean verifyed = true;
		try {
			PublicKey[] installedAppPubKeys = getInstalledAppPublicKey(context, packageName);
